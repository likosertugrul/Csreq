...
export async function PUT(req: NextRequest) {
  ...
  const db = getDb();
  db.prepare(`
    INSERT INTO user_profiles (user_id, name, hometown, purpose, interests, notes, home_text, refs_text, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  ...
  `).run(userId, name, hometown ?? "", purpose ?? "", interests ?? "", notes ?? "", homeText ?? "", refsText ?? "", Date.now());
